...
parser.add_argument('-bamdir', type=str, metavar='bam_directory', required=True, help='REQUIRED: Full path to directory with input bam files')
parser.add_argument('-o', type=str, metavar='output_directory', required=True, help='full path to output directory [aligned/]')
...
for bam in bam_list:
    ...
    sh_file.write('#!/bin/bash -e\n' +
                  ...
                  '#SBATCH --mem=' + args.mem + '\n' +
                  'source samtools-1.3\n' +
                  'samtools mpileup -C ' + args.C + ' -f ' + args.R + ' -o ' + args.o + bamname + '.C' + args.C + args.bamdir + bam)
    sh_file.close()
