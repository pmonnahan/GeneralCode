...
parser.add_argument('-bamdir', type=str, metavar='bam_directory', required=True, help='REQUIRED: Full path to directory with input fastq files')
parser.add_argument('-o', type=str, metavar='output_directory', required=True, help='Realtive path to output directory [aligned/]')
...
for pop in Pops:
    ...
    sh_file.write('#!/bin/bash -e\n' +
                  ...
                  '#SBATCH --mem=' + args.mem + '\n' +
                  'source samtools-1.3\n' +
                  'samtools mpileup -C ' + args.C + ' -f ' + args. R + ' -o ' + args.o + pop + '.C' + args.C + ".Chr" + str(chrom) + '.pileup' + args.bamdir + pop + '1.sort.CRG.bam ' + args.bamdir + pop + '2.sort.CRG.bam ' + args.bamdir + pop + '3.sort.CRG.bam')
    sh_file.close()
